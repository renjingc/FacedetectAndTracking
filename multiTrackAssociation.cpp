...
void TrakerManager::doWork(Mat& frame)
{
	...
	cout << "检测到走过的人数:" << max << endl;
	
	int current_num = 0;
	if (_tracker_list.size() > detections.size())
	{
		cout << "当前检测到的人数：" << _tracker_list.size() << endl;
	}
	else
	{
		cout << "当前检测到的人数：" << detections.size() << endl;
	}
	double area=0.0,bili=0.0;
	for (size_t i = 0; i<detections.size(); i++)
	{
		area += (detections[i].width*detections[i].height);
	}
	cout << "当前检测到的人脸密度：" << area/(double)(frame.cols*frame.rows) << endl;
	cout << endl;
